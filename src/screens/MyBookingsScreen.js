        ...
        try {
            ...
            const BookingCard = ({ item }) => {
                ...
                if (item.notes && item.notes.trim().length > 0) {
                    ...
                    if (serviceTasks.includes('_OP:')) {
                        serviceTasks = serviceTasks.replace(/_OP:\d+(\.\d+)?_?\s*/, '').trim();
                    }
                } else {
                    ...
                    >
                        ...
                        <View style={styles.cardTopRow}>
                            ...
                            <View style={styles.petInfoContainer}>
                                <AppText style={styles.petName} type="heading" weight="bold">{item.petName || 'Pet'}</AppText>
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    {(item.status === 'confirmed' || item.status === 'in_progress') && item.otp && (
                                        <View style={[styles.otpMinimalContainer, { marginTop: 0, marginLeft: 8 }]}>
                                            <AppText style={[styles.otpMinimalLabel, { marginRight: 4 }]}>PIN</AppText>
                                            <View style={styles.otpHighlight}>
                                                <AppText style={styles.otpMinimalValue} weight="bold">{item.otp}</AppText>
                                            </View>
                                        </View>
                                    )}
                                </View>
                        ...
                        <View style={[styles.cardMiddleRow, { position: 'relative' }]}>
                            {/* Date Column */}
                            ...
                            {/* Expert Column */}
                            <View style={[styles.metaColumn, { paddingRight: 45 }]}>
                                ...
                                <View style={styles.metaTextContainer}>
                                    ...
                                    <AppText style={styles.metaValue} numberOfLines={1}>
                                        {item.vendorName || (['pending', 'awaiting_vendor'].includes(item.status) ? 'Assigning Vendor...' : 'Unassigned')}
                                    </AppText>
                                </View>
                            </View>
                        </View>
